...
import { fetchTaskData, fetchProjectData, postData, fetchData } from 'src/store/apps/timesheets/index'
...
const TimesheetsDay = ({ popperPlacement }) => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchProjectData()).then(unwrapResult).then(() => {
      setLoading(false)
    })
  }, [])
  ...
  return (
    <div>
      {
        isLoading ? <FallbackSpinner /> : <Card>
          <CardContent>
            <Grid container spacing={2} alignItems='center'>
              ...
              <Grid item md={2}>
                ...
                {selectedValueError && selectedValue === '' && (
                  <FormHelperText sx={{ color: 'error.main', position: 'absolute' }}>Please select a value</FormHelperText>
                )}{' '}
              </Grid>
              ...
              <Grid item md={5}>
                <Autocomplete
                  ...
                  value={selectedProjectTasks?.find(option => option?.description === description) || null}
              ...
              <Grid item md={2}>
                ...
                />{" "}
                {timeError && timeInput === '' && (
                  <FormHelperText sx={{ color: 'error.main', position: 'absolute' }}>Please fill Time</FormHelperText>
                )}{' '}
              </Grid>
              <Grid item md={1}>
