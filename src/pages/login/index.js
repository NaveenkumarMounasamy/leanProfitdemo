...
const LoginPage = () => {
  ...
  const onSubmit = async data => {
    ...
    // const response = await axios.post(endpointURL(endpoints.login), data)

    // if (response.data.access) {
    //   window.localStorage.setItem("accessToken", response.data.access)
    //   const userData = jwt.decode(response.data.access, { complete: true }).payload
    //   window.localStorage.setItem("userData", JSON.stringify(userData))
    //   router.push("/apps/timesheets")
    // } else {
    //   toast.error("Login Failed")
    // }

    router.push(`//projects/details/task`)
  }
