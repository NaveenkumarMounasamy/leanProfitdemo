...
import Toolbar from 'src/views/leave-management/user/toolBar'
...
const UserLeaveManagement = () => {
  ...
  const [rows, setRows] = useState([{
    ...
    date: new Date().toDateString(),
    status: 'Approved',
    approver: 'Naveenkumar Mounasamy',
  ...
  const columns = [
    ...
    },
    {
      ...
      field: 'status',
      renderCell: (params) => {
        return <CustomChip size='small' skin='light' color={params.value === "Approved" ? "success" : params.value === "Rejected" ? "error" : "warning"} label={params.value} />
      }
    },
    {
      flex: 0.2,
      ...
      renderCell: ({ row, value }) => {
        return <Box sx={{ display: 'flex', flexDirection: 'row' }}>
          <CustomAvatar src={row.image} skin='light' color="primary" sx={{ mr: 3, fontSize: '.8rem', width: '1.875rem', height: '1.875rem' }}>
  ...
  return (<>
    ...
    {
      isLoading ? <FallbackSpinner /> : <Card><DataGrid
        autoHeight
        pagination
        rows={rows}
