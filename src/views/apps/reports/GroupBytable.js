...
export const GroupedTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      dispatch(fetchUserData()).then(unwrapResult).then(() => {
        tableRowHandler();
      })
    };
    fetchData().catch((e) => e);
    ...
    await _clients
      ...
      .forEach((client, i) => {
        client.project.forEach((p, j) => {
          p.tasks.flatMap(t => t.timeSheet).forEach((ts, k) => {
            clientReports.push({
              ...ts,
    ...
    await _users
      ...
      .forEach((user, i) => {
        user.projects.forEach((p, j) => {
          p.tasks.flatMap(o => o.timesheets).forEach((ts) => {
            userReports.push({
              ...ts,
              projectName: p.name,
              isBillable: ts.isBillable,
  ...
  const customizeAndMapProjects = async (row, _projects) => {
    ...
    await _projects.flatMap(o => o.tasks).flatMap(t => t.timesheet).forEach((ts, j) => {
      projectReports.push({
        ...ts,
        isBillable: ts.isBillable,
        date: ts.timesheetEntryDate,
