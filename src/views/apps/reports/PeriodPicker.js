...
import { fetchClientReports, fetchEmployeeReports, fetchProjectReports, setDateRanges } from 'src/store/apps/reports'
...
const PickersRange = ({ popperPlacement }) => {
  ...
  const handleOnChangeRange = dates => {
    ...
    if (start != null && end != null) {
      ...
      store.groupByValue === "Project" ? dispatch(fetchProjectReports(_dates)) : store.groupByValue === "Client" ? dispatch(fetchClientReports(_dates)) : dispatch(fetchEmployeeReports(_dates))
    }
  }
  ...
  return (
    <Box sx={{ display: 'flex', flexWrap: 'wrap' }} className='demo-space-x'>
      ...
      <DatePicker
        ...
        customInput={<CustomInput label='Date Range Filters' end={endDateRange} start={startDateRange} />}
      />
    </Box>
  )
}
