...
const CustomSkillPicker = ({ items, label }) => {
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', mt: 3 }}>
        {
          selectedValues.map((selectedValue, key) => {
            return <Chip key={key} color='primary' variant="filled" onDelete={() => handleDelete(selectedValue)} label={selectedValue.name} size="small" sx={{ m: 1.5 }} />
          }
          )
        }
