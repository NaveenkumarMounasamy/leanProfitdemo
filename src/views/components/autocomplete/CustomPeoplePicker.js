      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', mt: 3 }}>
        {
          selectedValues.map((selectedValue, key) => {
            return <Chip key={key} avatar={<Avatar src={`/images/avatars/${key + 1}.png`} />} color='primary' variant="outlined" onDelete={() => handleDelete(selectedValue)} label={selectedValue.name} size="small" sx={{ m: 1.5 }} />
          }
