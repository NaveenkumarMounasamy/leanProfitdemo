...
const TabHolidayManagement = ({ popperPlacement }) => {
    ...
    const handleDateChange = (date, params) => {
        ...
        if (date instanceof Date && !isNaN(date)) {
            let testing = {
                ...params, row: { ...params.row, date: dayjs(date).format('YYYY-MM-DD') }
            }
    ...
    const handleChange = (value, params) => {
        ...
        if (value != "" && event.target.value != undefined) {
            let testing = {
                ...params, row: { ...params.row, leaveDescription: value, date: dayjs(params.row.date).format('YYYY-MM-DD') }
            }
        ...
        },
        {
            ...
            renderEditCell: params => (
                <TextField
                    ...
                    fullWidth
                    onBlur={(e) => { handleChange(leaveDescription, params) }}
            ...
            renderCell: ({ row }) => (
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Tooltip title='Delete Holiday'>
                        <IconButton size='small' onClick={() => { dispatch(deleteHoliday([row])) }}>
